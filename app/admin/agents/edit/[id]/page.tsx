...
export default function EditAgentPage({ params }: EditAgentPageProps) {
  ...
  // Set form state when agent is loaded
  useEffect(() => {
    if (agent) {
      setName(agent.name || "");
      setDescription(agent.description || "");
      setWhatsapp(agent.whatsapp || "");
      setWhatsappGroup(agent.whatsappGroup || "");
      setWechat(agent.wechat || "");
      setWechatGroup(agent.wechatGroup || "");
      setTelegram(agent.telegram || "");
      setTelegramGroup(agent.telegramGroup || "");
      setTelephone(agent.telephone || "");
      setFacebookPage(agent.facebookPage || "");
      setFacebookGroup(agent.facebookGroup || "");
    ...
    return (
      <div className="container mx-auto py-8 px-4">
        ...
        <Card>
          ...
          <CardContent>
            <p>{error instanceof Error ? error.message : "Failed to load agent details"}</p>
          </CardContent>
          <CardFooter>
      ...
      <Card>
        <form onSubmit={handleSubmit}>
          <CardHeader>
            ...
            <CardDescription>
              Update agent information and contact details
            </CardDescription>
          </CardHeader>
